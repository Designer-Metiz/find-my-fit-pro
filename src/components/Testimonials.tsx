      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mx-auto max-w-2xl text-center">
          <h2 className="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
            What <span className="text-[#f97316]">Our Customers</span> Say
          </h2>
                ...
                return (
                  <Card 
                    ...
                    className={`transition-all duration-500 w-80 h-[450px] m-0 ${
                      isActive 
                        ? 'bg-[#2a2273] text-white shadow-2xl z-10' 
                        : isNext || isPrev
                  ...
                  >
                    <CardContent className="p-6 text-center h-full flex flex-col justify-center">
                      ...
                      <div className={`w-20 h-20 mx-auto mb-6 rounded-full flex items-center justify-center border-4 aspect-square ${
                        isActive ? 'bg-white text-[#2a2273] border-white' : 'bg-gradient-to-br from-[#2a2273] to-purple-600 text-white border-white/20'
                      }`} style={{ borderRadius: '50%' }}>
                        <span className={`text-xl font-bold ${
                          isActive ? 'text-[#2a2273]' : 'text-white'
                        }`}>
                ...
                {testimonials.map((_, index) => (
                  <button
                    ...
                    className={`w-3 h-3 rounded-full transition-colors ${
                      index === currentIndex ? 'bg-[#2a2273]' : 'bg-gray-300'
                    }`}
                  />
                ))}
              </div>
